-- @materialize: true
select 
  epoch_ms(CAST(COALESCE(a.createdAt, b.createdAt, c.createdAt) AS INT64)) AS event_datetime, 
  ...
  * EXCLUDE(createdAt, title, pageId), 
  ...
  CAST(extract('second' FROM event_datetime - LAG(event_datetime) OVER (PARTITION BY COALESCE(a.pageId, b.pageId, c.pageId), SPLIT(eventType, '-')[1] ORDER BY event_datetime)) AS FLOAT) AS duration,
from events a
